...
public class FileBinaryContentRepository implements BinaryContentRepository {
    ...
    public BinaryContent save(BinaryContent binaryContent) {
        Path path = getPath(binaryContent.getId());
        try {
            objectMapper.writeValue(path.toFile(), binaryContent);
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }
    ...
    public Optional<BinaryContent> findById(UUID uuid) {
        ...
        try {
            return Optional.of(objectMapper.readValue(path.toFile(), BinaryContent.class));
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }
    }
    ...
    public List<BinaryContent> findAll() {
        try (Stream<Path> stream = Files.list(DIRECTORY)) {
            return stream
                    ...
                    .map(path -> {
                        try {
                            return objectMapper.readValue(path.toFile(), BinaryContent.class);
                        } catch (IOException e) {
                            throw new UncheckedIOException(e);
                        }
                    })
